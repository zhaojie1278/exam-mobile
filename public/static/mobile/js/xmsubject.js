$(function(){
    ...
    $('.pagination a').click(
        function() {
            ...
            var sub_id = $(this).attr('for_sub_id');
        ...
        function(index) {
            ...
            if (undefined == $("input:radio[name='"+rd_name+"']:checked", ".weui-form").val() 
                || null == $("input:radio[name='"+rd_name+"']:checked", ".weui-form").val()) {
                    if (!is_commitsub) {
                        $.alert("请做完本页题，再继续下页题目，如果有不确定的，您可以先[<span style='color: blue'>标注一下</span>]");
                    } else {
                        $.alert("请做完本页题，再提交试卷");
                    }
                    check_rs = false;
